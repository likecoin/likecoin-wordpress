...
class LikeCoin_Admin_Utils {
	...
	public static function load_editor_scripts() {
		$sidebar_js_files  = LikeCoin_Admin_View::get_js_files( 'sidebar/index' );
		$sidebar_css_files = LikeCoin_Admin_View::get_css_files( 'sidebar/index' );

		wp_enqueue_script(
			'likecoin-sidebar-js',
			LC_URI . $sidebar_js_files,
			array( 'wp-i18n', 'wp-element', 'wp-blocks', 'wp-editor', 'wp-components', 'wp-api-fetch' ),
			LC_PLUGIN_VERSION,
			true
		);

		if ( $sidebar_css_files ) {
			wp_enqueue_style(
				'likecoin-sidebar-css',
				LC_URI . $sidebar_css_files,
				array( 'wp-components' ),
				LC_PLUGIN_VERSION
			);
		}
